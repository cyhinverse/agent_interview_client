...
export default function AISHowcaseCanvas() {
  ...
  useEffect(() => {
    ...
    const leetContent = [
      "Sliding Window", "Two Pointers", "Fast & Slow", "BFS", "DFS", 
      "Backtracking", "Dynamic Programming", "Greedy", "Trie",
      "Two Sum", "3Sum", "Add Two Numbers", "Valid Parentheses",
      "Merge Intervals", "Top K Elements", "Subsets", "Binary Search",
      "Tree Traversal", "Graph Search", "LRU Cache", "Two Heaps"
    ]
    ...
    const initItems = () => {
      ...
      for (let i = 0; i < 35; i++) {
        items.push({
          ...
          type: Math.random() > 0.85 ? 'panel' : (Math.random() > 0.2 ? 'leet' : 'point'),
    ...
    const drawWave = (wave: Wave, index: number, time: number) => {
      ...
      for (let i = 0; i < canvas.width; i++) {
        const offset = Math.sin(i * wave.length + wave.phase + time * wave.speed) * wave.amplitude
    ...
    const draw = (time: number) => {
      ...
      items.forEach(item => {
        ...
        if (item.type === 'panel') {
          ...
          ctx.beginPath()
          ctx.moveTo(10, 20); ctx.lineTo(item.size - 10, 20)
      ...
      const grad = ctx.createRadialGradient(canvas.width, 0, 0, canvas.width, 0, canvas.width)
