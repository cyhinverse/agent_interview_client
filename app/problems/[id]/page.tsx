...
export default function ProblemDetailPage() {
    ...
    examples: [
      {
        ...
        explanation: "The system waited for the majority and consolidated the context window correctly."
      }
    ],
    constraints: [
  ...
  return (
    <div className={`min-h-[calc(100vh-6rem)] flex flex-col pt-4 transition-all duration-300 ${isFullScreen ? "fixed inset-0 z-[60] bg-background pt-6 pb-6" : ""}`}>
      {/* Top Navigation / Breadcrumbs */}
      <div className={`${isFullScreen ? "px-10" : "max-w-7xl mx-auto px-6"} w-full mb-6`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <span className={`text-[10px] font-mono px-2 py-0.5 rounded-full border ${
              problem.difficulty === 'High' ? 'border-red-500/20 text-red-500 bg-red-500/5' : 'border-blue-500/20 text-blue-500 bg-blue-500/5'
            }`}>
              {problem.difficulty}
            </span>
          </div>
      ...
      {/* Main Content Area */}
      <div className={`flex-1 grid grid-cols-1 lg:grid-cols-2 gap-px bg-border ${isFullScreen ? "w-full border-y border-border" : "max-w-7xl mx-auto w-full border border-border rounded-xl"} overflow-hidden shadow-2xl bg-card`}>
          ...
          <div className="p-8 space-y-10">
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="p-4 rounded-xl bg-muted/40 border border-border/50 space-y-2">
                ...
                <div className="text-sm font-semibold">{problem.memoryLimit}</div>
              </div>
              <div className="p-4 rounded-xl bg-muted/40 border border-border/50 space-y-2">
                <div className="flex items-center gap-2 text-muted-foreground text-[10px] uppercase tracking-widest font-bold">
                  <Trophy className="w-3 h-3" /> Points
                </div>
                <div className="text-sm font-semibold text-primary">{problem.points}</div>
              </div>
            </div>
            ...
            <section className="space-y-6">
              <h4 className="text-sm font-bold uppercase tracking-wider text-foreground">Example Observations</h4>
              {problem.examples.map((ex, i) => (
                <div key={i} className="space-y-3">
                  <div className="p-4 rounded-lg bg-muted border border-border font-mono text-xs space-y-3 shadow-inner">
                    <div className="text-muted-foreground/60"># Input Queue</div>
                    <div className="text-foreground">{ex.input}</div>
                    <div className="h-px bg-border/50" />
                    <div className="text-muted-foreground/60"># Expected Output</div>
            ...
            <section className="space-y-3 pb-8">
              <h4 className="text-sm font-bold uppercase tracking-wider text-foreground">Mission Constraints</h4>
            ...
            <div className="opacity-80 space-y-1">
              {Array.from({ length: 25 }).map((_, i) => (
                <div key={i} className="flex gap-6">
                  <span className="w-6 text-right text-zinc-700 select-none text-xs">{i + 1}</span>
                  <div className="text-zinc-400">
                    {i === 0 && <span className="text-blue-400">class</span>} {i === 0 && <span className="text-yellow-400">ProtocolSolver</span>}: 
                    {i === 1 && <span className="pl-6 text-blue-400">def</span>} {i === 1 && <span className="text-yellow-400">solve_consensus</span>}(self, agents, latency):
                    {i === 2 && <span className="pl-12 text-zinc-600"># Implement orchestration logic here</span>}
